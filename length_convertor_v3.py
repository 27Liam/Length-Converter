...
class Converter:
    ...
    def __init__(self):
        ...
        button_details_list = [
            ["To Kilometers","#990099",lambda: self.check_length(c.ABS_ZERO_KILOMETERS), 0, 0],
            ["To Miles", "#009900", lambda: self.check_length(c.ABS_ZERO_MILES), 0, 1],
    ...
    def convert(self, min_length, to_convert):
        ...
        if min_length == c.ABS_ZERO_KILOMETERS:
            answer = cr.to_kilometers(to_convert)  # ← Now converts TO kilometers
            answer_statement = f"{to_convert} Miles is {answer} Kilometers"
        elif min_length == c.ABS_ZERO_MILES:
            answer = cr.to_miles(to_convert)  # ← Now converts TO miles
