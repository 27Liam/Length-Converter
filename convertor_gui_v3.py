...
class Converter:
    ...
    def __init__(self):
        ...
        # Set up GUI Frame
        length_frame = Frame(padx=10, pady=10)
        length_frame.grid()

        # Set up Widgets
        heading_label = {"text": "Length Converter", "font": font_16_b}
        heading_label_grid = {'row': 0}
        self.heading_label = Label(length_frame, heading_label).grid(heading_label_grid)

        instructions_label = {"text": "Instructions go here", "wraplength": 350}
        instructions_label_grid = {'row': 1}
        self.instructions_label = Label(length_frame, instructions_label).grid(instructions_label_grid)

        # Create entry widget
        self.temp_entry = Entry(length_frame, font=font_14)
        self.temp_entry.grid(row=2, padx=10, pady=10)

        # Conversion, help and history / export buttons
        button_frame = Frame(length_frame)
